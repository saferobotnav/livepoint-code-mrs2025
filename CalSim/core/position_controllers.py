...
from .dynamics import Qrotor3D, PlanarQrotor
...
class QRotorCBFCLFR2(Controller):
    def __init__(self, observer, lyapunovBarrierList, trajectory, depthCam):
        """
...
class QRotorCBFQPR2(CBF_QP_R2_3D_Ctrl):
    ...
    """
    def __init__(self, observer, lyapunovBarrierList, trajectory, depthCam, delta = 0.1, m = None, g = None):
        """
        Init function for a CBF-CLF-QP Controller. This controller works over the 
        relative degree 2 dynamics of the system and directly outputs a force
        vector, rather than a velocity vector to track.
        ...
        #create pointcloud and DepthProc objects to interface with the depth camera
        if isinstance(observer.dynamics, PlanarQrotor):
        ...
        elif isinstance(observer.dynamics, Qrotor3D):
    ...
    def update_weight(self, previous_value, sensitivity, liveness):
        """
        Update weights based on liveness and sensitivity.
        Parameters:
        - liveness: float, current liveness value
        - sensitivity: float, sensitivity parameter (n)
        """
        comp = liveness - np.pi/4
        new_value = previous_value - sensitivity * comp
...
class QRotor3DTracking(Controller):
    """
