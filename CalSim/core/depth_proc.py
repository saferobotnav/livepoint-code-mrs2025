...
class DepthProc:
    ...
    def closest_distance_to_static_obstacle(self, robot_point):
        """
        Calculates the closest distance from a robot point to the surface of the nearest static obstacle.
        
        Parameters:
            robot_point (numpy.ndarray): The position of the robot as a 3x1 column vector.
            
        Returns:
            float: The distance to the surface of the closest spherical obstacle.
        """
        # Ensure robot_point is a 1D array for consistency in calculations
        robot_point = robot_point.flatten()
        ...
        for center in sphere_centers:
            ...
            distance_to_center = np.linalg.norm(robot_point - center)  # Euclidean distance to center
            distance_to_surface = max(0, distance_to_center - sphere_radius)  # Distance to surface of sphere
            closest_distance = min(closest_distance, distance_to_surface)
